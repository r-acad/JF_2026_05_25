...
include(joinpath(REPO_ROOT, "JFEM_installation", "julia_tools", "run_manifest.jl"))
include(joinpath(REPO_ROOT, "JFEM_installation", "julia_tools", "manifest_batch_core.jl"))
...
function handle(req::HTTP.Request)
    ...
    # Restricting to vendor/ (rather than all of this app folder) avoids serving source
...
function serve(; host="127.0.0.1", port=8088)
    ...
    let si = _sysimage_info()
        if si["custom"]
            ...
            _log("  sysimage: NO (default Julia image)  -  run JFEM_installation/CLICK_WINDOWS_INSTALL_PACKAGES_AND_CREATE_SYSIMAGE.cmd or RUN_LINUX_INSTALL_PACKAGES_AND_CREATE_SYSIMAGE.sh for fast startup")
        end
