# Finite-difference guard for TACS PCOMP Tsai-Hill ply-failure sensitivities.
#
# The guard uses a compact unsymmetric MAT8 PCOMP laminate with MAT8 strength
# allowables and checks SOL101 KS ply-failure response derivatives for PCOMP
# ply thickness and ply angle variables against full plus/minus static re-solves.
#
# Usage:
#   julia --project=. tools/testing/tacs_pcomp_tsai_hill_failure_sensitivity_check.jl
...
function _write_pcomp_failure_deck(path::AbstractString)
    ...
    open(path, "w") do io
        println(io, "SOL 101")
        println(io, "CEND")
        println(io, "TITLE = TACS PCOMP Tsai-Hill ply failure sensitivity guard")
        ...
        println(io, "MAT8,1,1.35E11,9.0E9,0.28,5.0E9,4.0E9,3.6E9,1600.,0.,0.,0.,1.8E9,1.2E9,6.0E7,2.0E8,9.0E7")
...
function main()
    tmp = mktempdir(; prefix="openjfem_tacs_pcomp_tsai_hill_failure_")
    deck = _write_pcomp_failure_deck(joinpath(tmp, "pcomp_tsai_hill_failure.bdf"))
    ...
    println("TACS PCOMP Tsai-Hill failure sensitivity check passed")
