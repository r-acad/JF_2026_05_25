# Finite-difference guard for TACS PCOMP Tsai-Wu ply-failure sensitivities.
#
...
#
# Usage:
#   julia --project=. tools/testing/tacs_pcomp_tsai_wu_failure_sensitivity_check.jl
...
function _write_pcomp_failure_deck(path::AbstractString)
    ...
    open(path, "w") do io
        println(io, "SOL 101")
        println(io, "CEND")
        println(io, "TITLE = TACS PCOMP Tsai-Wu ply failure sensitivity guard")
...
function main()
    tmp = mktempdir(; prefix="openjfem_tacs_pcomp_tsai_wu_failure_")
    deck = _write_pcomp_failure_deck(joinpath(tmp, "pcomp_tsai_wu_failure.bdf"))
    ...
    @test gradient["criterion"] == "tsai_wu"
    ...
    println("TACS PCOMP Tsai-Wu failure sensitivity check passed")
