# Finite-difference guard for TACS-style modified Tsai-Wu PCOMP failure sensitivities.
#
# The modified Tsai-Wu form is the strength-ratio scalar used by TACS'
# orthotropic ply default. This guard checks SOL101 KS ply-failure response
# derivatives for PCOMP ply thickness and ply angle variables against full
# plus/minus static re-solves.
#
# Usage:
#   julia --project=. tools/testing/tacs_pcomp_modified_tsai_wu_failure_sensitivity_check.jl
...
function _write_pcomp_failure_deck(path::AbstractString)
    ...
    open(path, "w") do io
        println(io, "SOL 101")
        println(io, "CEND")
        println(io, "TITLE = TACS PCOMP modified Tsai-Wu ply failure sensitivity guard")
...
function main()
    tmp = mktempdir(; prefix="openjfem_tacs_pcomp_modified_tsai_wu_failure_")
    deck = _write_pcomp_failure_deck(joinpath(tmp, "pcomp_modified_tsai_wu_failure.bdf"))
    ...
    @test gradient["criterion"] == "modified_tsai_wu"
    ...
    println("TACS PCOMP modified Tsai-Wu failure sensitivity check passed")
