...
function _solve_sol105(model, cc, K, K_eig, id_map, X, ndof, node_R,
    ...
    # construct an options object explicitly.
    ...
    # subcase. Each carries the per-subcase Kg / K_eig / u_static / fixed_dofs
    # so off-line MAC / Rayleigh-quotient parity doesn't have to spin up
    # per-subcase BDFs to recover them.
    subcase_results = Solver.BucklingSubcaseResult[]
    last_Kg = spzeros(ndof, ndof)
    ...
    for (buck_sid, stat_sid) in buckling_subcases
        ...
        push!(subcase_results, Solver.BucklingSubcaseResult(
            ...
            Dict{String,Float64}(
                ...
                "kg_assembly"            => Float64(kg_wall_seconds),
                "buckling_eigensolve"    => Float64(buck_wall_seconds),
            ),
            begin
                # Phase C1: include the resolved SOL105Options in the
                # per-subcase details dict so reports / HDF5 export carry the
                # exact knob state used to produce this result. Reproducibility
                # footer — does not change behavior.
    ...
    return Dict(
        ...
        # K_eig/Kg/u_static/fixed_dofs) for backwards compat.
        "buckling" => Solver.BucklingResult(subcase_results),
        "mesh" => mesh,
        "model" => model,
        "id_map" => id_map,
        "node_coords" => X,
